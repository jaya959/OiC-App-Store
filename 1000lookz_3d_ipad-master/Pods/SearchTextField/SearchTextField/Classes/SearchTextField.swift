...
open class SearchTextField: UITextField {
        ...
        didSet {
            ...
            if let placeholderColor = theme.placeholderColor {
                if let placeholderString = placeholder {
                    ...
                    self.attributedPlaceholder = NSAttributedString(string: placeholderString, attributes: [NSForegroundColorAttributeName: UIColor.gray])
    ...
    // open var highlightAttributes: [NSAttributedStringKey: AnyObject] = [.zone: UIFont.boldSystemFont(ofSize: 10)]
    ...
    fileprivate var normalValues = [SearchTextFieldItem]()
    ...
    fileprivate func buildPlaceholderLabel() {
        ...
        if let placeholderLabel = placeholderLabel {
            ...
            //            if let placeholderColor = self.attributedPlaceholder?.attribute([NSForegroundColorAttributeName: UIColor.gray], at: 0, effectiveRange: nil) as? UIColor {
    ...
    fileprivate func filter(forceShowAll addAll: Bool) {
        ...
        for i in 0 ..< filterDataSource.count {
            ...
            //                        item.attributedSubtitle!.setAttributes(highlightAttributesForSubtitle() as [String : Any], range: subtitleFilterRange)
            //                    }
            //
            ...
            if inlineMode, let filterAfter = startFilteringAfter {
                if let suffixToFilter = textToFilter.components(separatedBy: filterAfter).first, (suffixToFilter != "" || startSuggestingInmediately == true), textToFilter != suffixToFilter {
...
extension SearchTextField: UITableViewDelegate, UITableViewDataSource {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if (text?.isEmpty)! {
            cell!.textLabel?.text = normalValues[(indexPath as NSIndexPath).row].title.capitalizedFirst()
        }else{
            
            cell!.textLabel?.text = filteredResults[(indexPath as NSIndexPath).row].title.capitalizedFirst()
        }
    ...
    public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if itemSelectionHandler == nil {
            ...
            if (self.text?.isEmpty)!{
                ...
                self.text = normalValues[(indexPath as NSIndexPath).row].title
