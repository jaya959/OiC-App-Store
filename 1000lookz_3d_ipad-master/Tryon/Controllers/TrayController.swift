...
//        } else {
//            self.searchTextFields.text = defaultAddressText
//        }
    }
...
//        } else {
//            accountDropDownButton.isHidden = false
//            accountDropDownButton.isUserInteractionEnabled = true
//            searchTextFields.isHidden = true
//            searchTextFields.isUserInteractionEnabled = false
//        }
...
//                total = total + (price * Double(frame.orderQuantityCount))
//            }
            if let qty:Int = frame.orderQuantityCount {
...
//            self?.accountDropDownButton.setTitle(self?.defaultAccountText, for: .normal)
//            self?.selectedAccountId = nil
//            self?.accountDropDown.reloadAllComponents()
           self?.searchTextFields.text = ""
        }
            ...
            for i in 0..<TrayHelper().trayInventoryFramesCount() {
                ...
                if self?.selectedPrice == "DP"{
                    if let price = frame[i].priceDistributor.value {
                        calcPriceText = self?.numberFormatter.string(from: NSNumber(value: price * Double(frame[i].orderQuantityCount)))
    ...
    func itemLoadArray() {
        ...
        for i in 0..<TrayHelper().trayInventoryFramesCount(){
            ...
            if self.selectedPrice == "DP"{
                if let price = frame.priceDistributor.value {
                    calcPriceText = numberFormatter.string(from: NSNumber(value: price * Double(frame.orderQuantityCount)))
                }
            } else {
                if let price = frame.price.value {
                    calcPriceText = numberFormatter.string(from: NSNumber(value: price * Double(frame.orderQuantityCount)))
...
//                if string == "Arcadio_f" {
                    self.searchTextFields.text = ""
//                } else {
//                    self.accountDropDownButton.setTitle(defaultAccountText, for: .normal)
//                }
            }
            else {
                self.showAlertMessage(withTitle: "Oops", message: "Select Location.")
                    ...
                    TrayHelper().addItemsToCart(orderbyId: self.model.userId, accountId: accountId, frameIds: frameIds, quantity: quantity, orderDatetime: time as Date, orderStatusId: 1, completionHandler: { (orderDetails, error) in
                        if error == nil {
                            TrayHelper().placeOrder(orderbyId: self.model.userId, accountId: accountId, itemDetails: orderDetails, orderDatetime: Date(), orderStatusId: 2, comment: self.commentTextView.text , completionHandler: { (result, error) in
                                ...
                                // print(result[0])
                                // print(result[0].value(forKeyPath: "orderby_.address1")! as! String)
                                self.invoiceNumber = result[0].value(forKey: "account_order_id") as? Int
//                                let name:String = result[0].value(forKeyPath: "orderby_.name")! as! String
//                                if let address2:String = (result[0].value(forKeyPath: "orderby_.address1") as? String){
//                                    address1 = address2
//                                }
//                                let district: String = result[0].value(forKeyPath: "orderby_.district")! as! String
//                                let pincode: String = result[0].value(forKeyPath: "orderby_.pincode")! as! String
//                                let address: String = name + "<br>" + address1 + "<br>" + district + "-" + pincode
...
//                                        let string = String(describing: UserDefaults.standard.value(forKey: "UserName")!)00
//                                        if string == "Arcadio_f" {
//                                            self.searchTextFields.text = ""
//                                        } else {
//                                            self.accountDropDownButton.setTitle(self.defaultAccountText, for: .normal)
...
extension TrayController: TrayDelegate {
    func removeDidTap(trayCell: TrayCell) {
        ...
        let _ = TrayHelper().addInventoryFrameToTray(TrayHelper().trayInventoryFrames()[(indexPath?.row)!])
